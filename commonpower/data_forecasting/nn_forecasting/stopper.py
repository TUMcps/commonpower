"""
Stopper classes for early stopping in training.
"""
...
class TrialPlateauOverfitStopper(Stopper):
    """Early stop single trials when they reached a plateau or start overfitting.
    ...
    def __init__(
        ...
        self._counter = 0
        self._best_performance = float("inf") if mode == "min" else -float("inf")
    ...
    def __call__(self, trial_id: str, result: Dict):
        ...
        # Check if we need to update the best performance
        if self._mode == "min":
            if metric_result < self._best_performance:
                self._best_performance = metric_result
            else:
                self._counter += 1
        elif self._mode == "max":
            if metric_result > self._best_performance:
        ...
        # If counter exceeds the number of results, stop early.
        return self._counter >= self._num_results or current_std < self._std
