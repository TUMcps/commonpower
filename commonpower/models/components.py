...
from commonpower.modelling import ModelElement, ElementTypes as et, MIPExpressionBuilder
...
class Load(Component):
    ...
    def _get_model_elements(cls) -> List[ModelElement]:
        model_elements = [
            ModelElement("p", et.DATA, "active power", pyo.NonNegativeReals),
            ModelElement("q", et.DATA, "reactive power", pyo.Reals),
        ]
...
class RenewableGen(Component):
    ...
    def _get_model_elements(cls) -> List[ModelElement]:
        model_elements = [
            ModelElement("p", et.DATA, "active power", pyo.NonPositiveReals),
        ]
        return model_elements
...
class RenewableGenCurtail(Component):
    ...
    def _get_model_elements(cls) -> List[ModelElement]:
        model_elements = [
            ...
            ModelElement("p_pot", et.DATA, "active power potential", pyo.NonPositiveReals),
            ModelElement("p", et.VAR, "active power (after curtailing)", pyo.NonPositiveReals),
        ]
        return model_elements
...
class _EV_Episodic(Component):
    """
    DEPRECIATED. We can use EV for episodic control just as well (with the benefit of considering a return time).
    EVs have a charge requirement which needs to be fullfiled by a certain deadline in each control cycle.
    This component is only suitable for episodic control because once the deadline is reached and the EV is unplugged,
    it does not consider its return.

    The constraints are:
    is_plugged_in <- deadline > t
    is_charged <- soc >= req_soc
    1 <- is_plugged_in OR is_charged
    ...
    def _get_model_elements(cls) -> List[ModelElement]:
        model_elements = [
            ...
            ModelElement("rho", et.CONSTANT, "cost of wear pu", pyo.NonNegativeReals),
            ModelElement("etac", et.CONSTANT, "charge efficiency", bounds=[0.0, 1.0], domain=pyo.NonNegativeReals),
            ModelElement("etad", et.CONSTANT, "discharge efficiency", bounds=[0.0, 1.0], domain=pyo.NonNegativeReals),
            ModelElement(
                "etas", et.CONSTANT, "self-discharge coefficient", bounds=[0.0, 1.0], domain=pyo.NonNegativeReals
            ),
            # we have to adjust the deadline after each control step (-1)
            ModelElement("deadline", et.CONSTANT, "deadline (time step)", domain=pyo.NonNegativeIntegers),
            ModelElement(
                "req_soc_rel",
                et.CONSTANT,
    ...
    def _get_additional_constraints(self) -> List[ModelElement]:
        ...
        mb.from_gt("deadline", [x for x in range(self.horizon + 1)], "is_plugged_in")

        # either is_charged and/or is_plugged_in must be true -> Make sure the EV is charged by the deadline
        mb.enforce_value(mb.from_or("is_charged", "is_plugged_in"), 1)
        ...
        return [req_soc_abs, c_req_soc_abs] + mb.model_elements + [c_ev_unplugged]  # this order is necessary

    def cost_fcn(self, model: ConcreteModel, t: int = 0) -> Expression:
        """
        The cost represents: cost = abs(p) * rho
        """
        return (
            (
                self.get_pyomo_element("p", model)[t] * (self.get_pyomo_element("p_ec", model)[t])
...
class HeatPumpWithoutStorageButCOP(Component):
    ...
    def cost_fcn(self, model: ConcreteModel, t: int = 0) -> Expression:
        ...
        return (
            self.get_pyomo_element("c", model)
            * (self.get_pyomo_element("T_indoor", model)[t] - self.get_pyomo_element("T_indoor_setpoint", model)) ** 2
            * self.tau
        )
    ...
    def _get_dynamic_fcn(self) -> List[ModelElement]:
        ...
        def dynamic_fcn(model, t):
            ...
            if t == self.horizon:
                ...
                return (
                    self.get_pyomo_element("soc", model)[t]
                    + self.get_pyomo_element("p", model)[t] * self.tau
                    == self.get_pyomo_element("soc", model)[t + 1]
                )
